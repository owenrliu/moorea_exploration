...
fringing reef, backreef, and forereef. One way to start to investigate this
...
for (i in 1:length(sp_rep.names)) {
  fr.dat <- exp.counts(sp_rep,sp_rep.names[i],'FR') #fringing reef data expansion
  br.dat <- exp.counts(sp_rep,sp_rep.names[i],'BA') #back reef data expansion
  fo.dat <- exp.counts(sp_rep,sp_rep.names[i],'FO') #forereef data expansion

  dat.sp <- rbind(fr.dat,br.dat,fo.dat) #combined
  dat.all[[sp_rep.names[i]]] <- dat.sp
}
```
